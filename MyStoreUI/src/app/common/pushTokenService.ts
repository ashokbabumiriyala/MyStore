...
export class PushTokenService {
  ...
  token: string;
  private registerProviderPushTokenUrl: string;
  constructor(
    private commonApiServiceCallsService: CommonApiServiceCallsService
  ) {
    this.registerProviderPushTokenUrl =
      environment.pushTokenServiceUrl + 'UpdateProviderPushToken';
  }

  public registerProviderPushToken(
    providerId: number,
    token: string
  ): Observable<any> {
    return this.commonApiServiceCallsService
      .select(this.registerProviderPushTokenUrl, {
