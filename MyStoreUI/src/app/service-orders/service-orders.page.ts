...
export class ServiceOrdersPage implements OnInit {
  ...
  async serviceOrderList() {
    ...
    await this.serviceOrderService
      .serviceOrdersSelect('ServiceOrdersList', dataObject)
      .subscribe(
        (data: any) => {
  ...
  async getServiceOrderItems(orderId: string) {
    ...
    await this.serviceOrderService
      .serviceOrdersItemsSelect('ServiceOrderItemList', dataObject)
      .subscribe(
        (data: any) => {
          this.serviceOrderedItems = data.servicesOrderItems.sort(
            (a, b) => <any>new Date(b.orderDate) - <any> new Date(a.orderDate)
          );
